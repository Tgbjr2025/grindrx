...
pockets.append(((np.array(sh_inner) > 127), 3.0))
...
for bxc, byc, br in BALLS:
    ...
    inside = rr <= 1.0
    cap = 3.0 * (1 - np.sqrt(np.clip(1 - rr ** 2, 0, 1)))
...
eng_mask(dm & mimask, 2.0); occupied |= dm
...
for bxc, byc, br in BALLS:
    for sgn in (-1, 1):
        ...
        st2.arc([ac - br, byc - br, ac + br, byc + br],
                100 if sgn > 0 else -80, 260 if sgn > 0 else 80, fill=255, width=mm(0.8))
eng_mask((np.array(stitch2) > 127), 1.6)
...
np.save('tu_final_eng.npy', final_eng)
np.save('tu_crest_plate.npy', crest_plate)
np.save('tu_sh_inner.npy', (np.array(sh_inner) > 127))
np.save('tu_Dmask.npy', Dmask)
np.save('tu_head.npy', head_m)
np.save('tu_face_m.npy', face_m)
np.save('tu_letters.npy', (np.array(letters) > 127))
np.save('tu_compass.npy', cm)
np.save('tu_banner_plates.npy', (np.array(plates) > 127))
np.save('tu_rimg.npy', rmg)
np.save('tu_roads.npy', rm)
np.save('tu_dots.npy', dm & mimask)
np.save('tu_trees.npy', tm)

# ------------------------------------------------------------------ output
depth = np.clip(depth, 0, MAXD)
gray = np.round(255 * (1 - depth / MAXD)).astype(np.uint8)
Image.fromarray(gray).save('david_lakes_depth_v2.png')
...
Image.fromarray(img).save('david_lakes_preview_v2.png')
